...
void parseMapsFile(char *maps_path, char *prog_path)
{
  ...
  if(fseek(fs, 73, SEEK_SET) == -1) {
    fprintf(stderr, RED "Failed to operate on the file: %s" CLEAR "\n", strerror(errno));
    exit(EXIT_FAILURE);
  }
  fgets(readbuf, BUF_LENGTH, fs);
  rewind(fs);
  ...
  if(!strncmp(basename_prog, basename(readbuf), strlen(basename_prog))) {
    ...
    fprintf(stderr, RED "File described in \"maps\" do not match the tracee" CLEAR "\n");
...
void printLayout(pid_t pid, char* prog_path)
{
  ...
  if(sprintf(maps_path, "/proc/%d/maps", pid) < 0)
    fprintf(stderr, RED "Error parsing the path to 'maps' file" CLEAR "\n");
...
void adjustRegisters(pid_t pid, Dwarf_Addr sym_addr, long orig_data, long trap)
{
  ...
  if(ptrace(PTRACE_GETREGS, pid, NULL, &registers) == -1) {
    fprintf(stderr, RED "Failed to get registers information: %s" CLEAR "\n", strerror(errno));
    exit(EXIT_FAILURE);
  }

  if(ptrace(PTRACE_POKEDATA, pid, sym_addr, orig_data) == -1) {
    fprintf(stderr, RED "Failed to poke data: %s" CLEAR "\n", strerror(errno));
    exit(EXIT_FAILURE);
  }
  ...
  if(ptrace(PTRACE_SINGLESTEP, pid, NULL, NULL) == -1) {
    fprintf(stderr, RED "Failed to continue the tracing: %s" CLEAR "\n", strerror(errno));
    exit(EXIT_FAILURE);
  }
  wait(NULL);
...
void getOpt(int argc, char** argv)
{
  ...
  while(opt != -1) {
    if(opt == 'a')
      ...
      fprintf(stderr, RED "Error: option %c not recognized" CLEAR "\n", optopt);
      exit(EXIT_FAILURE);
    }
...
int main(int argc, char** argv)
{
  ...
  if(sym_addr == 0) {
    fprintf(stderr, RED "Error: symbol not found" CLEAR "\n");
    exit(EXIT_FAILURE);
  }

  if(opt_verbose) {
    printf(GREEN "Symbol found" CLEAR "\n");
    printf(YELLOW "Breakpoint at %llx" CLEAR "\n", sym_addr);
  }

  pid_child = fork();
  if(pid_child == -1) {
    fprintf(stderr, RED "Failed to fork: %s" CLEAR "\n", strerror(errno));
    exit(EXIT_FAILURE);
  }
  if (pid_child == 0) {
    if(ptrace(PTRACE_TRACEME, 0, NULL, NULL) == -1) {
      fprintf(stderr, RED "Failed to start tracing: %s" CLEAR "\n", strerror(errno));
      exit(EXIT_FAILURE);
    }
    if(execv(prog_path, argv + optind + 1) == -1) {
      fprintf(stderr, RED "Failed to start the tracee: %s" CLEAR "\n", strerror(errno));
      exit(EXIT_FAILURE);
    }
    exit(EXIT_SUCCESS);
  }
  ...
  if(opt_verbose)
    printf(YELLOW "Beginning of the analysis of process %d (%s)" CLEAR "\n", pid_child, prog_path);
  ...
  if(orig_data == -1) {
    if(errno != 0) {
      fprintf(stderr, RED "Failed to peek data: %s" CLEAR "\n", strerror(errno));
      exit(EXIT_FAILURE);
    }
  }
  ...
  while(1) {
    ...
    if (WIFSTOPPED(status) && WSTOPSIG(status) == SIGTRAP) {
      if(opt_verbose)
        printf(YELLOW "Hit the breakpoint %llx" CLEAR "\n", sym_addr);
    ...
    if(WIFEXITED(status)) {
      if(opt_verbose)
        printf(YELLOW "Completed tracing pid %d" CLEAR "\n", pid_child);
      break;
    }
  }
